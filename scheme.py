...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        value = procedure.apply_macro(rest,env)
        return scheme_eval(value, env)
    else:
       
        "*** YOUR CODE HERE ***"
        current_eval = scheme_eval(first,env)
        operands = rest.map(lambda x: scheme_eval(x,env))
        return scheme_apply(current_eval, operands, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        symbol, function_parameters = target.first, target.rest
        value = do_lambda_form(Pair(function_parameters, expressions.rest),env)
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        curr_val = scheme_eval(bindings.first.rest.first,env)
        vals = Pair(curr_val,vals)
...
def do_define_macro(expressions, env):
    ...
    macro_procedure = expressions.first
    target = macro_procedure.first
    formals= macro_procedure.rest
    body = expressions.rest
    if scheme_symbolp(target):
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result,Thunk):
            ...
            new_env = result.env
            result = original_scheme_eval(expression,new_env)
